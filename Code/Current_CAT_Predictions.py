...
df = pd.DataFrame(CAT_Last_Month_Preds)
df.to_csv("Data/CAT_Last_Month_Preds.csv")
...
plt.plot(date_sequence, CAT_Last_Month_Preds, color = 'r')
plt.plot(Last_Month, color = 'b', label = "CAT Stock Last Month")
plt.legend()
ax.set_ylabel("Closing Price (stationary)")
ax.set_xlabel("Date")
ax.set_title("LSTM Predictions on CAT Stock")
plt.xticks(rotation = 45, ha = 'right', fontsize = 6)
ax.xaxis.set_major_locator(MaxNLocator(nbins=31))
fig.tight_layout()
plt.savefig("Figures/CAT_Last_Month.png")
plt.clf()
